...
export class Point {
  ...
  /**
   * Compute the inner product with another point.
   *
  ...
  /**
   * Compute the Z-component of the outer product with another point.
   *
   * @param {Point} point - A point object.
   * @returns {number} The Z-component of the outer product.
   */
  outerProd(point) {
  ...
  /**
   * Rotate the point around the specified center.
   *
  ...
  /**
   * Scale the point for each direction.
   * If you want to scale only the length of the point vecotr, you should use {@link Point#scale} instead.
   *
   ...
   * @returns {Point} The scaled point object.
   */
  scale2(ratioX, ratioY, center = new Point(0, 0)) {
    return new Point(
      (this.x - center.x) * ratioX + center.x,
      (this.y - center.y) * ratioY + center.y
    );
  }
}
...
export class Rectangle {
  ...
  /**
   * Checks if both the width and height of the rectangle are 0.
   *
  ...
  /**
   * Checks if a point is on the edge of the rectangle.
   *
   * @param {Point} point - A point object to be checked.
   * @returns {boolean} `true` if the point is on the edge.
   */
  hasOnEdge(point) {
    const x = (point.x === this.left() || point.x === this.right())
      && (this.top() <= point.y && point.y <= this.bottom());
    const y = (point.y === this.top() || point.y === this.bottom())
  ...
  /**
   * Checks if the rectangle contacts to another rectangle.
   *
   * @param {Rectangle} rect - An rectangle object to check contact.
   * @returns {boolean} `true` if two rectangles contact.
   */
  contacts(rect) {
    const x = (this.left() === rect.right() || rect.left() === this.right())
