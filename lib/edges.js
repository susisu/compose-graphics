...
export function selfIntersections(edge, depth, maxIteration = -1) {
  ...
  const splits = [];
  {
    ...
    for (let i = 1; i < eps.length - 1; i++) {
      ...
      const specials = t === 0 ? [{ t: 0, point: edge.start.clone() }] : [];
      splits.push({ edge: e1, t, ratio: ep.t - t, specials });
      current = e2;
      t = ep.t;
    }
    {
      const specials = [{ t: 1, point: edge.end.clone() }];
      splits.push({ edge: current, t, ratio: 1 - t, specials });
    }
  }
  ...
  for (let i = 0; i < splits.length; i++) {
    ...
    for (let j = i + 1; j < splits.length; j++) {
      ...
      const rs = _intersectionsWithSpecialPoints(
        s1.edge, s2.edge, s1.specials, s2.specials, depth, maxIteration
      );
      if (rs === undefined) {
        return undefined;
      }
