...
export class Line {
   ...
   */
  boundingBox() {
    const xs = [this.start.x, this.end.x];
    const ys = [this.start.y, this.end.y];
...
export class CubicBezier {
  ...
  boundingBox() {
    ...
    if (ax !== 0) {
      ...
      if (0 <= tx1 && tx1 <= 1) {
        xs.push(this.pointAt(tx1).x);
      }
      if (0 <= tx2 && tx2 <= 1) {
        xs.push(this.pointAt(tx2).x);
      }
    }
    else {
    ...
    if (ay !== 0) {
      ...
      if (0 <= ty1 && ty1 <= 1) {
        xs.push(this.pointAt(ty1).y);
      }
      if (0 <= ty2 && ty2 <= 1) {
        xs.push(this.pointAt(ty2).y);
      }
    }
    else {
