...
export function intersections(edge1, edge2, iterations) {
  ...
  while (queue.length > 0) {
    ...
    const err = Math.max(0.5 ** (i + 1), Number.EPSILON);
    const delta = 0.5 ** (i + 2);
    switch (type) {
    case PT.PP:
      {
    ...
    case PT.PE:
      {
        ...
        const contained = bb2.contains(p1);
        const onEdge = bb2.hasOnEdge(p1);
        if (contained || onEdge) {
          if (i >= iterations) {
            res.push(mr(t1, t2, onEdge ? 2 * err : Math.SQRT2 * err));
            break;
          }
          const p2 = e2.pointAt(0.5);
          const [e21, e22] = e2.splitAt(0.5);
          queue.push(mp(i + 1, PT.PP, [p1, p2], t1, t2));
    ...
    case PT.EP:
      {
        ...
        const contained = bb1.contains(p2);
