...
function simeq(x, y, minEps = 0) {
  const eps = Math.max(Number.EPSILON * Math.max(Math.abs(x), Math.abs(y)), minEps);
  return x === y || Math.abs(x - y) < eps;
}

// checks if two points are very close
function simeqPoints(p1, p2, minEps = 0) {
  return simeq(p1.x, p2.x, minEps) && simeq(p1.y, p2.y, minEps);
}
...
function _intersectionsWithSpecialPoints(
  edge1, edge2, specialPoints1, specialPoints2, depth, resolution = 0, maxIteration = -1
) {
  let itr = 0;
  ...
  while (queue.length > 0) {
    ...
    case PT.PP:
      {
        const [p1, p2] = pair;
        if (simeqPoints(p1, p2)
          && !exactRes.some(r => simeq(r.t1, t1, resolution) && simeq(r.t2, t2, resolution))) {
          const r = mr(t1, t2, 0);
          res.push(r);
          exactRes.push(r);
        }
    ...
    case PT.EE:
      {
        ...
        if (bb1.isPoint()) {
          ...
          if (bb2.contains(p1)) {
            queue.push(mp(i, PT.PE, [p1, e2], t1, t2));
          }
          break;
        }
        if (bb2.isPoint()) {
          ...
          if (bb1.contains(p2)) {
            queue.push(mp(i, PT.EP, [e1, p2], t1, t2));
          }
          break;
        }
        if (bb1.overlaps(bb2)) {
...
function mergeNeighboringPoints(res, resolution = 0) {
  ...
  for (let i = 0; i < res.length; i++) {
    ...
    for (let j = i + 1; j < res.length; j++) {
      ...
      const r2 = res[j];
      const err = Math.max(Math.SQRT2 * (r1.err + r2.err), resolution);
      const close = simeq(r1.t1, r2.t1, err) && simeq(r1.t2, r2.t2, err);
      if (close) {
        if (r1.err > r2.err) {
          removed[i] = true;
        }
 ...
 * @param {number} [resolution=0] - Minimum difference in t value of distinct intersections.
 * @param {number} [maxIteration=-1] - Maximum number of iterations. No limitation if negative.
 * @returns {Array<{ t1: number, t2: number, point: Point }>|undefined} - An array of the intersections.
 * If there are infinitely many intersections, it returns `undefined`.
 */
export function intersections(edge1, edge2, depth, resolution = 0, maxIteration = -1) {
  ...
  const res = _intersectionsWithSpecialPoints(
    ...
    depth, resolution, maxIteration
  );
  if (res === undefined) {
    return undefined;
  }
  return mergeNeighboringPoints(res, resolution)
 ...
 * If there are infinitely many intersections, it returns `undefined`.
 */
export function selfIntersections(edge, depth, resolution = 0, maxIteration = -1) {
  const res = [];
